...
describe('Database, Sequelize and Models', () => {
	...
	describe('Job Model', () => {
		...
		test('should have 7 jobs created by user with id 5', async () => {
			const jobCreated = await Job.findAll({
				raw: true,
				where: { UserId: 5 }
			});
	...
	describe('Skill Model', () => {
		...
		test('should have the correct value', async () => {
			...
			expect(skill).toEqual(
				expect.arrayContaining(listOfSkills)
			);
		});
	});
	...
	describe('Application Model', () => {
		test('should have 8 users that applied to jobs', async () => {
			...
			const appliedUser = users.filter(
				user => !user['Jobs.company']
			);
	...
	describe('JobSkill Model', () => {
		beforeAll(async () => {
			...
			const swift = skills.find(
				skill => skill.name === 'Swift'
			);
			const dart = skills.find(
				skill => skill.name === 'Dart'
			);
			const golang = skills.find(
				skill => skill.name === 'Golang'
			);
			const angular = skills.find(
				skill => skill.name === 'Angular'
			);
			const cPlus = skills.find(
				skill => skill.name === 'C++'
			);
		...
		test('should have skills added to Disney and The Home Depot', async () => {
			...
			const disneySkills = disney.Skills.map(
				skill => skill.name
			);
			const homeDepotSkills = homeDepot.Skills.map(
				skill => skill.name
			);
		...
		test('should have 3 skills for Photosnap job', async () => {
			const photosnap = await Job.findOne({
				where: {
					company: 'Photosnap'
				}
			});
			const photoSkills = await photosnap.getSkills();
			const listOfSkills = photoSkills.map(
				skill => skill.name
			);
